                    ...
                    <el-table-column
                       ...
                       -->
                        <template slot-scope="scope">
                            ...
                            -->
                            <el-popover v-for="role in scope.row.roles" :key="role.id" trigger="hover" placement="left">
    ...
    created() {
        ...
        methods: {
            ...
            getUserList() {
                ...
                /**
                 * TODO:去后台查询数据：
                 *  请求参数：select * from 表名 limit 起始索引,每页条数;
                 *          1.当前页码
                 *          2.每页条数
                 *          起始索引=(当前页码-1)*每页条数
                 *  响应处理：
                 *      1.将后端响应的数据放到userList: [] 显示用户和角色信息
                 *      2.将后台查询的用户表的总记录数的值赋值给数据模型中的：pagination中的total既可以显示分页栏
                 *            pagination: {
                                total: 0,  //总条数
                                pageNum: 1, // //当前页
                                pageSize: 5 //每页显示条数
                ...
                */
                ...
                //2.定义变量保存请求参数 {"currentPage":this.pagination.pageNum,"pageSize":this.pagination.pageSize}
                ...
                axios.post(url,params)
                     .then(resp=>{
                        ...
                        if(obj.flag){
                            ...
                            //7.将查询的用户和角色信息赋值给数据模型中的userList: []，然后vue就会自动注入到表格中
                            this.userList=obj.result.rows;
                            //8.将后台查询的用户表的总记录数的值赋值给数据模型中的：pagination中的total既可以显示分页栏
